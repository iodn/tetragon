...
Arguments, as specified in the `args:` list, are made available in the CEL expression as `argX` where `X` is the zero-based index in the list. Similary for data items, as specified in the `data:` list.
